...
export const ArticleRow = ({ onClick, title, meta, variant = 'index', children }: ArticleRowProps) => {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full text-left bg-transparent cursor-pointer border-t-[0.5px] border-stone-200 dark:border-stone-700 flex flex-col gap-4 group transition-colors hover:border-stone-500 dark:hover:border-stone-400 ${isEssay ? 'py-12 md:py-14' : 'py-9 md:py-10'}`}
    >
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline gap-2 sm:gap-4">
        ...
        <span
          className={
            isEssay
              ? 'newsreader italic text-[12px] sm:text-[13px] text-stone-500 dark:text-stone-500 whitespace-nowrap'

...
- 源码在 `academic-portfolio/`（React + Vite）
- 页面内容在 `academic-portfolio/content/`（按板块拆分 `.md`）
...
- 修改 CV：`academic-portfolio/content/cv.md`
- 修改项目：`academic-portfolio/content/projects/*.md`
- 修改论文：`academic-portfolio/content/publications/*.md`
- 修改 Zen Land：`academic-portfolio/content/zen/*.md`
...
- 首页模块：`academic-portfolio/views/HomeSections.tsx`
- 页面容器：`academic-portfolio/views/PortfolioViews.tsx`
- 导航与全局状态：`academic-portfolio/App.tsx`
- 个人信息、新闻、研究方向、技能等结构化数据：`academic-portfolio/constants.tsx`
...
# 生产构建（输出到 academic-portfolio/dist）
npm run build
...
- `npm run bootstrap`：执行 `npm --prefix academic-portfolio ci`
...
- 页面内容改了但线上没更新  
  先确认你修改的是 `academic-portfolio/content/` 或源码目录，而不是根目录产物。

- 本地运行端口不是 3000  
  查看 `academic-portfolio/vite.config.ts` 中 `server.port` 配置。
...
- Source code: `academic-portfolio/` (React + Vite)
- Content files: `academic-portfolio/content/` (modular `.md` files)
...
- Update CV: `academic-portfolio/content/cv.md`
- Update projects: `academic-portfolio/content/projects/*.md`
- Update publications: `academic-portfolio/content/publications/*.md`
- Update Zen Land posts: `academic-portfolio/content/zen/*.md`
...
- Home modules: `academic-portfolio/views/HomeSections.tsx`
- View container: `academic-portfolio/views/PortfolioViews.tsx`
- Navigation/global state: `academic-portfolio/App.tsx`
- Profile/news/research/skills structured data: `academic-portfolio/constants.tsx`
...
# Production build (output: academic-portfolio/dist)
npm run build
...
- `npm run bootstrap`: runs `npm --prefix academic-portfolio ci`
...
1. Install dependencies and build `academic-portfolio`
...
- I changed content but the live site did not update  
  Make sure you edited files under `academic-portfolio/content/` or source files, not root-level build artifacts.

- Local dev server is not using port 3000  
  Check `server.port` in `academic-portfolio/vite.config.ts`.

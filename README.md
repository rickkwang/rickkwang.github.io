...
- 源码在 `portfolio/`（React + Vite）
- 页面内容在 `portfolio/content/`（按板块拆分 `.md`）
...
- 修改 CV：`portfolio/content/cv.md`
- 修改项目：`portfolio/content/projects/*.md`
- 修改论文：`portfolio/content/publications/*.md`
- 修改 Zen Land：`portfolio/content/zen/*.md`
...
- 首页模块：`portfolio/views/HomeSections.tsx`
- 页面容器：`portfolio/views/PortfolioViews.tsx`
- 导航与全局状态：`portfolio/App.tsx`
- 个人信息、新闻、研究方向、技能等结构化数据：`portfolio/constants.tsx`
...
- `npm run bootstrap`：执行 `npm --prefix portfolio ci`
...
- 页面内容改了但线上没更新  
  先确认你修改的是 `portfolio/content/` 或源码目录，而不是根目录产物。

- 本地运行端口不是 3000  
  查看 `portfolio/vite.config.ts` 中 `server.port` 配置。
...
- Source code: `portfolio/` (React + Vite)
- Content files: `portfolio/content/` (modular `.md` files)
...
- Update CV: `portfolio/content/cv.md`
- Update projects: `portfolio/content/projects/*.md`
- Update publications: `portfolio/content/publications/*.md`
- Update Zen Land posts: `portfolio/content/zen/*.md`
...
- Home modules: `portfolio/views/HomeSections.tsx`
- View container: `portfolio/views/PortfolioViews.tsx`
- Navigation/global state: `portfolio/App.tsx`
- Profile/news/research/skills structured data: `portfolio/constants.tsx`
...
# Production build (output: portfolio/dist)
npm run build
...
- `npm run bootstrap`: runs `npm --prefix portfolio ci`
...
1. Install dependencies and build `portfolio`
...
- I changed content but the live site did not update  
  Make sure you edited files under `portfolio/content/` or source files, not root-level build artifacts.

- Local dev server is not using port 3000  
  Check `server.port` in `portfolio/vite.config.ts`.
